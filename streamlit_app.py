    ...
    if not len(projects):
        ...
        if filtered_project_df.empty:
            ...
            chart = alt.Chart(chart_df).mark_line().encode(
                ...
                color=alt.Color('Type', legend=alt.Legend(title="Spend Type")),
                tooltip=['Project Name', 'Date', 'Type', alt.Tooltip('Amount', format='$,.2f')]
            ).properties(
                title='Budget vs Actual Spend Over Time'
